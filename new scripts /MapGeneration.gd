...
func _ready():
    ...
    if Global.saved_map_state != null:
        print("Obnovuji mapu...")
...
func generate_loop_path() -> void:
    ...
    var y = start_y
    map_grid[x][y] = 1
    path_tilemap.set_cell(0, Vector2i(x, y), CAMPFIRE_TILE_ID, Vector2i(0, 0), 0) # pozice Atlas souřadnic ID1
